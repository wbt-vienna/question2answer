...
*/
	...
	class q2apro_stopspam_captcha 
	{
		...
		function form_html(&$qa_content, $error)
		{
			...
			$html .= '<label for="captcha_a" class="captcha_q">'.qa_opt('q2apro_stopspam_captcha_question').'</label>';
		  	$html .= '<input id="captcha_a" class="form-control" type="text" name="captcha_challenge_field" />';
			$html .= '<input type="checkbox" name="response_field" id="response_field" />';
			$html .= '</div>';
			// insert css to hide the honeypot
			$html .= '<style type="text/css"> .captcha_q{margin-right:10px; font-weight: normal} #response_field{display:none;} .q2apro_captcha { margin-bottom:20px; }</style>';
		...
		function validate_post(&$error)
		{
			...
			else if(!empty($_POST['captcha_challenge_field']))
			{
				...
				if( strcasecmp($_POST['captcha_challenge_field'],qa_opt('q2apro_stopspam_captcha_answer')) == 0
					|| strcasecmp($_POST['captcha_challenge_field'], qa_opt('q2apro_stopspam_captcha_answer2')) == 0 )
				{
					return true;
				}
				else 
